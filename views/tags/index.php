<?php
  ...
  if ($app->user->isAdmin()) {
    $tag = $this->dbConn->stdQuery("SELECT `tags`.`id` FROM `tags` ORDER BY `tags`.`name` ASC LIMIT ".((intval($app->page)-1)*$resultsPerPage).",".intval($resultsPerPage));
    $tagPages = ceil($this->dbConn->queryCount("SELECT COUNT(*) FROM `tags`")/$resultsPerPage);
  } else {
    $tag = $this->dbConn->stdQuery("SELECT `tags`.`id` FROM `tags` WHERE `approved_on` != '' ORDER BY `tags`.`name` ASC LIMIT ".((intval($app->page)-1)*$resultsPerPage).",".intval($resultsPerPage));
    $tagPages = ceil($this->dbConn->queryCount("SELECT COUNT(*) FROM `tags` WHERE `approved_on` != ''")/$resultsPerPage);
  }
?>
<h1>All Tags</h1>
<?php echo paginate($newTag->url("index", array("page" => "")), intval($app->page), $tagPages); ?>
...
?>
    <tr>
      ...
      <td><?php echo $app->user->isAdmin() ? $thisTag->link("edit", "Edit") : ""; ?></td>
      <td><?php echo $app->user->isAdmin() ? $thisTag->link("delete", "Delete") : ""; ?></td>
    </tr>
<?php
  }
?>
